...
import {
  ...
  updateCashEntry, updateCustomer, updatePayment, updateProfile, voidMovement,
  type ActionResult,
} from "@/app/actions";
...
export function DashboardShell({ profile, customers, collections, movements, chart, summary }: Props) {
  ...
  const removeMovement = async (movement: Movement) => {
    ...
    await run(voidMovement, "Movimentação excluída.")(data);
  };
  ...
  return <div className="app-shell">
    ...
    <main className="main-content">
      ...
      {view === "movements" && <section className="dashboard page-view">
        ...
        {movements.length ? <div className="movement-list">{movements.map((item) => <article className="movement-row" key={item.id}>
          <span className={`movement-icon ${item.kind}`}>{item.kind === "expense" ? <ArrowUpRight size={18} /> : <ArrowDownLeft size={18} />}</span>
          <div><strong>{item.label}</strong><span>{item.detail} · {item.isPaid === false && item.dueDate ? `vence ${dateLabel(item.dueDate)}` : dateLabel(item.date)}</span>{item.isPaid === false && <span className="pending-label">Pendente</span>}</div>
          <div className="movement-actions"><b className={item.kind}>{item.kind === "expense" ? "−" : "+"}{money(item.amountCents)}</b>{item.editable && <span>
            ...
            <button className="mini-action delete" aria-label={`Excluir ${item.label}`} disabled={isSaving} onClick={() => removeMovement(item)}><Trash2 size={15} /></button>
          </span>}</div>
    ...
    {modal && <div className="modal-backdrop" onMouseDown={close}><section className="register-sheet" role="dialog" aria-modal="true" onMouseDown={(event) => event.stopPropagation()}><div className="sheet-handle" /><div className="sheet-heading"><div><p className="eyebrow">{modal === "more" ? "Navegação" : "Registro rápido"}</p><h2>{modal === "menu" ? "O que você quer registrar?" : modal === "customer" ? "Nova cliente" : modal === "editCustomer" ? "Editar cliente" : modal === "sale" ? "Nova venda" : modal === "cash" ? "Movimentação do caixa" : modal === "editCash" ? "Editar movimentação" : modal === "editPayment" ? "Editar recebimento" : modal === "payExpense" ? "Pagar despesa" : modal === "payment" ? "Receber pagamento" : "Mais opções"}</h2></div><button className="icon-button" aria-label="Fechar" onClick={close}><X size={20} /></button></div>
      ...
      {modal === "editPayment" && selectedMovement?.sourceId && <form className="entry-form" action={run(updatePayment, "Recebimento atualizado.")}><div className="payment-summary"><strong>{selectedMovement.label}</strong><span>{selectedMovement.detail}</span></div><input type="hidden" name="id" value={selectedMovement.sourceId} /><label>Valor recebido<input name="amount" required autoFocus inputMode="decimal" defaultValue={(selectedMovement.amountCents / 100).toFixed(2).replace(".", ",")} /></label><label>Data do recebimento<input name="paid_on" type="date" required defaultValue={selectedMovement.date} /></label><label>Forma de pagamento<select name="method" defaultValue={selectedMovement.paymentMethod ?? "pix"}>{paymentOptions.map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label><button className="primary-button" disabled={isSaving}>{isSaving ? "Salvando..." : "Salvar alterações"}</button></form>}
